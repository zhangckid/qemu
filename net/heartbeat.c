...
{
    s->timeout_timer = timer_new_ms(QEMU_CLOCK_VIRTUAL,
                                    heartbeat_timeout,
                                    s);

    s->pulse_timer = timer_new_ms(QEMU_CLOCK_VIRTUAL,
                                  heartbeat_regular_pulse,
                                  s);
...
static void heartbeat_complete(UserCreatable *uc, Error **errp)
{
    ...
    if (!s->heartbeat_node || !s->pulse_interval || !s->timeout) {
        error_setg(errp, "heartbeat needs 'heartbeat_node' and 'interval' "
                   "'timeout' property set");
        return;
    }
